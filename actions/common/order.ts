...
export async function getProgress(
  tableData: TTableData & { date: { start: Date; end: Date } }
) {
  try {
    let totalData = 0,
      totalPage = 0;
    const session = await auth(),
      user = await prisma.user.findFirst({ where: { id: session?.user?.id } });
    if (user) {
      const data = await prisma.course
        ...
        .then((res) => {
          const today = new Date(),
            startDate = tableData.date.start.toISOString(),
            endDate = tableData.date.end.toISOString();
          return res.map(({ order, incomeRate, ...rest }) => {
            const thisMonthData = order.filter(
              ...
              filteredData = order.filter((v) => {
                const currentDate = v.date.toISOString();
                return (
