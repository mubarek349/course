...
import { StateType, TAffiliate, TTableData } from "@/lib/definations";
...
export async function getAffiliates(
  ...
  try {
    let totalData = 0,
      totalPage = 0;
    const date = {
        start: new Date(tableData.date.start?.toString() ?? null)
          .toISOString()
          .slice(4, 15),
        end: new Date(tableData.date.end?.toString() ?? null)
          .toISOString()
          .slice(4, 15),
      },
      today = new Date();
              ...
              res.map(async (value) => {
                const order = await prisma.order.findMany({
                  ...
                  filteredData = order.filter(
                    (v) =>
                      ((v.date?.toString()?.slice(4, 15) ?? "") > date.start &&
                        (v.date?.toString()?.slice(4, 15) ?? "") < date.end) ||
                      ((v.date?.toString()?.slice(4, 15) ?? "") < date.start &&
                        (v.date?.toString()?.slice(4, 15) ?? "") > date.end)
                  ),
...
export async function removeAffiliate(
  ...
  try {
    await prisma.user.delete({ where: { role: "affiliate", id } });
    return { status: true };
  } catch {
    return { status: false, cause: "", message: "" };
  }
